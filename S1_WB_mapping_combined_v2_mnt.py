import rsgislib, os, gdal, time, argparse, sys, numpy, rios, os.path
...
guf='/mnt/Data/Andy/Projects/Zambia/Supporting_data/GUF_Barotseland_snapped_large.kea' # global urban footprint
permWater='/mnt/Data/Andy/Projects/Zambia/Supporting_data/classified_stack_2017_occurrence_v2_perm_water.kea' # permanent water layer
globalWater='/mnt/Data/Andy/Projects/Zambia/Supporting_data/seasonality_barotseland_snapped.tif' # global water seasonality layer
otherMask='/mnt/Data/Andy/Projects/Zambia/Supporting_data/barotseland_srtm_utm_lee_slope_gt1_5_wgs84.shp' # training data for dry areas
...
if (d >= 2) & (d <= 6): # select dry season images based on image month
	...
	condition='(permWat==1)?1:class'
	outClass=outimage.replace('.kea','_refined.kea')
	gdalformat = 'KEA'
	imagecalc.bandMath(outClass, condition, gdalformat, rsgislib.TYPE_8UINT, bandDefns)
	...
	for file in glob.glob(os.path.split(inputImage)[0]+'/*wb*'):
		os.remove(file)
