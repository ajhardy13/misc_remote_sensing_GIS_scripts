...
###########################################################################
###### reset nan values as 50 so they form a unique object upon segmentation
nanOut=inputImage.replace('.tif','_nonan.tif')

# define output names for each band
b1=inputImage.replace('.tif','_b1.kea')
b2=inputImage.replace('.tif','_b2.kea')
b3=inputImage.replace('.tif','_b3.kea')
...
imagecalc.bandMath(b1_nan, '(b1==0.0)?50:b1', gdalformat, rsgislib.TYPE_32FLOAT, bandDefns)
bandDefns = []
bandDefns.append(BandDefn('b2', b2, 1))
imagecalc.bandMath(b2_nan, '(b2==0.0)?50:b2', gdalformat, rsgislib.TYPE_32FLOAT, bandDefns)
bandDefns = []
bandDefns.append(BandDefn('b3', b3, 1))
imagecalc.bandMath(b3_nan, '(b3==0.0)?50:b3', gdalformat, rsgislib.TYPE_32FLOAT, bandDefns)

# stack the nonan bands together
gdalformat='GTiff'
gdaltype = rsgislib.TYPE_32FLOAT
imageutils.stackImageBands([b1_nan,b2_nan,b3_nan], ['VV','VH','VVdivVH']  , nanOut, None, 50, gdalformat, gdaltype)
...
try:
	shutil.move(waterSnap, '/home/georgina/Desktop/MyData/Data/LINUX_WORK/Projects/Zambia/Sentinel_1/Intermediate_Files/'+waterSnap)
except Exception:
	pass
...
mode_cert[numpy.where((vvMax_val==50))]=0
...
try:
	shutil.move(outCert, '/home/georgina/Desktop/MyData/Data/LINUX_WORK/Projects/Zambia/Sentinel_1/Intermediate_Files/'+outCert)
except Exception:
	pass
...
if (d >= 2) & (d <= 5): # select dry season images based on image month
	...
	try:
		shutil.move(outimage.replace('.kea','_otsu.kea'), '/home/georgina/Desktop/MyData/Data/LINUX_WORK/Projects/Zambia/Sentinel_1/Intermediate_Files/' + outimage.replace('.kea','_otsu.kea'))
	except Exception:
		pass
	
	# remove all open water snapped images
	for file in glob.glob('*'+os.path.split(inputImage)[1].split('.')[0]+'*wb*'):
		shutil.move(file, '/home/georgina/Desktop/MyData/Data/LINUX_WORK/Projects/Zambia/Sentinel_1/Intermediate_Files/' + file)
...
# move nonan image
try:
	shutil.move(nanOut, '/home/georgina/Desktop/MyData/Data/LINUX_WORK/Projects/Zambia/Sentinel_1/Intermediate_Files/'+nanOut) 
except Exception:
	pass
