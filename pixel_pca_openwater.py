...
outMatrixFile=inputImage.replace('.kea','_eigen')
...
def performImagePCA(inputImg, outputImg, eigenVecFile, nComponents=None, pxlNSample=100, gdalformat='KEA', datatype=rsgislib.TYPE_32UINT, noData=None, calcStats=True):
	...
	eigenVec, varExplain = getPCAEigenVector(inputImg, pxlNSample, noData, eigenVecFile)
	...
	rsgislib.imagecalc.pca(inputImg, eigenVecFile, outputImg, outNComp)
	if calcStats:
		usenodataval=False
		nodataval=0
		if noData is not None:
...
eigenVectors=outMatrixFile
numComponents=2

performImagePCA(clumpsMean, outputImage, eigenVectors, nComponents=2, pxlNSample=200, gdalformat='KEA', datatype=rsgislib.TYPE_32UINT, noData=None, calcStats=True)

#rsgislib.imagecalc.performImagePCA(clumpsMean, outputImg, eigenVecFile, nComponents=2, pxlNSample=1, gdalformat='KEA', datatype=rsgislib.TYPE_32UINT, noData=None, calcStats=True)

#rsgislib.imagecalc.pca(eigenVectors, clumpsMean, outputImage, numComponents)
