import rsgislib, os, gdal, time, subprocess, argparse, sys, numpy, rios, collections
...
outimage=inputImg.split('.')[0]+'_clumps2_erf_clumptrain_mode_sel.tif'
...
sandMask='/mnt/Data/Andy/Projects/Zambia/Supporting_data/sel_hand_sand_mask_'+inputImg.split('/')[-1].split('_')[4]+'_clump_m18dB.shp'
...
#sandMask='/Users/Andy/Documents/Zambia/RemoteSensing/WB_classification/Supporting_data/sand_exclusion_layer/sel_hand_sand_mask_'+inputImg.split('/')[-1].split('_')[4]+'_clump_m18dB.shp'
##############################################################################################
...
def classify(outputClumps):
	...
	classesIntCol='classesIntColBal'
	# run the classifier
	classratutils.classifyWithinRAT(outputClumps, classesIntCol, classesNameCol, variables, classifier=classifier, classColours=None,preProcessor=MaxAbsScaler(),outColInt=outColInt)
...
#define clumps colours for final classification
...
'''
############################################################################################
#sieve the output to remove the small clumps that are unlikely to be water
