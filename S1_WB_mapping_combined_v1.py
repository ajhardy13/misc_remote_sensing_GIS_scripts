import rsgislib, os, gdal, time, argparse, sys, numpy, rios
import os.path
...
outimage=inputImage.split('.')[0]+'_classified.tif'

##############################################################################################
# ancialliary datasets
guf='/Users/Andy/Documents/Zambia/RemoteSensing/WB_classification/Supporting_data/global_urban_footprint/GUF_Barotseland_snapped.tif' # global urban footprint
...
otherMask='/Users/Andy/Documents/Zambia/RemoteSensing/WB_classification/Supporting_data/global_surface_water/ssh_out/barotseland_srtm_utm_lee_slope_gt1_5_wgs84.shp' # training data for dry areas
...
# function to extract water seasonality >2 and low VV/VH ratio (lt 0.45) 
...
vvMax_val=[]
vvMax_val.append(rat.readColumn(ratDataset, 'vvMax_val')) # read in urban footprint column
...
# where GUF > 100, and mode_cert==3 (vegWater) classify as dry
mode_cert[numpy.where((guf_val>100)&(mode_cert==3))]=2
...
d=int(inputImage.split('_')[4][4:6])
...
if (d >= 2) & (d <= 6): # select dry season images based on image month
	...
	inRefImg=inputImage
	outSnap=permWater.replace('.kea','_'+inputImage.split('_')[4]+'_snap.kea')
	gdalformat = 'KEA'
	...
	outClass=outimage.replace('.tif','_otsu.tif')
	gdalformat = 'GTiff'
	...
	outClass=outimage.replace('.tif','_refined.tif')
	gdalformat = 'GTiff'
	imagecalc.bandMath(outClass, condition, gdalformat, rsgislib.TYPE_8UINT, bandDefns)

	try:
		os.remove(outimage.replace('.tif','_otsu.tif'))
	except Exception:
		pass
