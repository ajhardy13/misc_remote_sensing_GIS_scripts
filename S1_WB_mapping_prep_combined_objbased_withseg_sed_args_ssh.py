...
sand_exclusion='/mnt/Data/Andy/Projects/Zambia/Supporting_data/sel_hand_sand_mask.kea'
...
#sand_exclusion='/Users/Andy/Documents/Zambia/RemoteSensing/WB_classification/Supporting_data/sand_exclusion_layer/sel_hand_sand_mask.kea'
...
##############################################################################################
# snap the sand exclusion layer 
##############################################################################################
print('Snapping sand mask...')

bandList=['SEL']
rsgislib.imageutils.setBandNames(sand_exclusion, bandList)

sand_exclusion_snap=sand_exclusion.split('.')[0]+'_'+inputImage.split('_')[-4]+'_clump.tif'

gdalFormat = 'GTiff'
inRefImg=inputImage
rsgislib.imageutils.resampleImage2Match(inRefImg, sand_exclusion, sand_exclusion_snap, gdalFormat, interpMethod='nearestneighbour', datatype=rsgislib.TYPE_8INT) # perform resampling/snap



##############################################################################################
##############################################################################################
# create sand training data
##############################################################################################
@jit
def sandmask(sand_in, radar_in, outName, outShp):
	...
	bandDefns.append(BandDefn('sand', sand_in, 1))
	
	# conditional statement to select permanent water with low backscatter
	print("Running conditional statement....")
	gdalformat = 'KEA'
	imagecalc.bandMath(outName, '(VV>-18)&&(VVVH>0.45)&&(sand==1)?1:0', gdalformat, rsgislib.TYPE_8UINT, bandDefns)


sand_in=sand_exclusion_snap
radar_in=inputImage.replace('.tif','_clumps2_mean.kea')
outName=sand_exclusion_snap.replace('.tif','_m18dB.tif')
outShp=sand_exclusion_snap.replace('.tif','_m18dB.shp')
print('Sand mask: ' +outShp)

sandmask(sand_in, radar_in, outName, outShp)
